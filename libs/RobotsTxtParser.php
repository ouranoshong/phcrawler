...
class RobotsTxtParser
{
     ...
     *
     * @param URLDescriptor $BaseUrl           The root-URL all rules from the robots-txt-file should relate to
     ...
     */
    public function parseRobotsTxt(URLDescriptor $BaseUrl, $user_agent_string, $robots_txt_uri = null)
    {
        Benchmark::start("processing_robotstxt");
        ...
        $robots_txt_content = Utils::getURIContent($robots_txt_uri, $user_agent_string);
        ...
        if ($robots_txt_content != null)
        {
            ...
            $non_follow_reg_exps = $this->buildRegExpressions($applying_lines, Utils::getRootUrl($BaseUrl->url_rebuild));
        }

        Benchmark::stop("processing_robots.txt");
    ...
    protected function buildRegExpressions($applying_lines, $base_url)
    {
        ...
        $normalized_base_url = Utils::normalizeURL($base_url);
