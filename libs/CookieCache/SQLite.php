...
class SQLite extends Base
{
     ...
     */
    public function addCookie(CookieDescriptor $Cookie)
    {
//        $source_domain = $Cookie->source_domain;
    ...
    {
        Benchmark::start("adding_cookies_to_cache");
        ...
        Benchmark::stop("adding_cookies_to_cache");
    }
    ...
    {
        Benchmark::start("getting_cookies_from_cache");

        $url_parts = Utils::splitURL($target_url);
        ...
        for ($x=0; $x<$cnt; $x++)
        {
            ...
            if ($rows[$x]["domain"] == $url_parts["host"] || preg_match("#".preg_quote($rows[$x]["domain"])."$#", $url_parts["host"]))
            {
                ...
                {
                    $Cookie = new CookieDescriptor($rows[$x]["source_url"], $rows[$x]["name"], $rows[$x]["value"], $rows[$x]["expires"], $rows[$x]["path"], $rows[$x]["domain"]);
        ...
        Benchmark::stop("getting_cookies_from_cache");
    ...
    {
        //Benchmark::start("Connecting to SQLite-cache-db");
        ...
        //Benchmark::stop("Connecting to SQLite-cache-db");
    }
