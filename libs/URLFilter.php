...
class URLFilter
{
     ...
     *
     * @var array The URL-parts as returned by Utils::splitURL()
     */
    ...
    /**
     * Current DocumentInfo-object of the current document
     *
     * @var DocumentInfo
     */
    ...
    public function setBaseURL($starting_url)
    {
        ...
        $this->starting_url_parts = Utils::splitURL($starting_url);
    }

    /**
     * Filters the given URLs (contained in the given DocumentInfo-object) by the given rules.
     *
     * @param DocumentInfo $DocumentInfo DocumentInfo-object containing all found links of the current document.
     */
    public function filterUrls(DocumentInfo $DocumentInfo)
    {
        Benchmark::start("filtering_urls");
        ...
        Benchmark::stop("filtering_urls");
    }

    /**
     * Filters out all non-redirect-URLs from the URLs given in the DocumentInfo-object
     *
     * @param DocumentInfo $DocumentInfo         DocumentInfo-object containing all found links of the current document.
     * @param bool                   $decrease_link_depths
     */
    public static function keepRedirectUrls(DocumentInfo $DocumentInfo, $decrease_link_depths = false)
    {
        $cnt = count($DocumentInfo->links_found_url_descriptors);
        for ($x=0; $x<$cnt; $x++)
        {
    ...
    protected function urlMatchesRules(URLDescriptor $url)
    {
