...
class SQLite extends Base
{
     ...
     *
     * @return URLDescriptor An URLDescriptor or NULL if currently no
     *                                 URL to process.
     */
    public function getNextUrl()
    {
        Benchmark::start("fetching_next_url_from_sqlitecache");
        ...
        Benchmark::stop("fetching_next_url_from_sqlitecache");

        // Return URL
        return new URLDescriptor($row["url_rebuild"], $row["link_raw"], $row["linkcode"], $row["linktext"], $row["refering_url"], $row["url_link_depth"]);
    }
     ...
     */
    public function addURL(URLDescriptor $UrlDescriptor)
    {
    ...
    {
        Benchmark::start("adding_urls_to_sqlitecache");
        ...
        Benchmark::stop("adding_urls_to_sqlitecache");
    }
     ...
     * @param URLDescriptor $UrlDescriptor
     */
    public function markUrlAsFollowed(URLDescriptor $UrlDescriptor)
    {
        Benchmark::start("marking_url_as_followes");
        $hash = md5($UrlDescriptor->url_rebuild);
        $this->PDO->exec("UPDATE urls SET processed = 1, in_process = 0 WHERE distinct_hash = '".$hash."';");
        Benchmark::stop("marking_url_as_followes");
    }
    ...
    {
        Benchmark::start("checking_for_urls_in_cache");
        ...
        Benchmark::stop("checking_for_urls_in_cache");
    ...
    {
        Benchmark::start("connecting_to_sqlite_db");
        ...
        Benchmark::stop("connecting_to_sqlite_db");
    }
